...
const Message: React.FC<MessageProps> = ({ message, isLive = false }) => {
  ...
  const bubbleClasses = `max-w-xs md:max-w-md px-4 py-3 rounded-2xl ${
    isUser
      ? 'bg-gradient-to-r from-aljeri-blue to-aljeri-green text-white font-medium ltr:rounded-br-none rtl:rounded-bl-none shadow-lg'
      : 'bg-gray-100 dark:bg-[#242938] text-gray-800 dark:text-white font-medium ltr:rounded-bl-none rtl:rounded-br-none shadow-md border border-gray-200/10'
  } ${isLive ? 'opacity-70' : ''}`;
